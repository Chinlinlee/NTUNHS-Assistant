...
let historyScoreChartApp = angular.module("historyScoreChartApp", ["commonApp"]);

historyScoreChartApp.controller('historyScoreChartCtrl', function ($scope, $timeout, historyScoreChartService, commonService) {
    $scope.courseList = [];
    ...
    $scope.$watch('selectedCourse' , function (newV , oldV) {
        ...
        for (let i = 0 ; i < newV.length ; i++) {
            let course = newV[i];
            $(`#scoreChart_${course.courseSem}_${course.courseNormalId}`).show();
            $(`#chartTitle_${course.courseSem}_${course.courseNormalId}`).show();
        }
    });
    ...
    $scope.query = function () {
        ...
        historyScoreChartService.getCourseScoreChart(qs).then(function (res) {
            if (res.status == 200) {
                $timeout(async function() {
                    ...
                    for (let i = 0 ; i < $scope.courseList.length ; i++) {
                        ...
                        let checkIsChangeView = setInterval(function () {
                            if ($(`#scoreChart_${course.courseSem}_${course.courseNormalId}`).is(":visible")) {
                                clearInterval(checkIsChangeView);
                                createChart(course , `scoreChart_${course.courseSem}_${course.courseNormalId}`)
                            }
                        } , 100)
                    }
                } , 0) 
