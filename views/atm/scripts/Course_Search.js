...
CSApp.controller("CSCtrl", function ($scope, CSService, commonService) {
    ...
    $scope.$watch("preModalQ", function (newValue, oldValue) {
        ...
        $scope.PreScheduleQueryDataList = $scope.PreScheduleQueryStaticDataList.filter((item) => {
    ...
    $scope.PreScheduleAddCondition = function (iItem, iDay) {
        ...
        CSService.PreScheduleQuery($scope, [iDay + 1], [iItem.Period]).then(function (res) {
            ...
            $scope.PreScheduleQueryStaticDataList = JSON.parse(JSON.stringify(res.data[0]));
