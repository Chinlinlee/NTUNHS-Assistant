...
async function getCourseExcel(driver) {
    return new Promise(async (resolve) => {
        await driver
            .navigate()
            .to(
                'http://system10.ntunhs.edu.tw/AcadInfoSystem/Modules/QueryCourse/QueryCourse.aspx'
            );
        await driver.executeScript(
            `$("#ContentPlaceHolder1_ddlSem").children().eq(1).attr("selected" ,true).change()`
        ); //高到低 1>當前 2> 上學期
        ...
        driver
            .wait(
                webdriver.until.elementIsVisible(
                    driver.findElement(
                        webdriver.By.id('ContentPlaceHolder1_imgExcel')
                    )
                ),
                150000
            )
            .then(async (p) => {
                await driver.executeScript(
                    `$("#ContentPlaceHolder1_imgExcel").click()`
                );
...
async function seleSchoolLogin(iDriver, acc, pwd) {
    return new Promise(async (resolve) => {
        try {
            iDriver.get(
                'http://system8.ntunhs.edu.tw/myNTUNHS_student/Modules/Main/Index_student.aspx?timeout=false'
            ); //打开https://autowebtest.github.io/
            iDriver.sleep(100);
            await iDriver.wait(
                webdriver.until.elementLocated({
                    id: 'ctl00_loginModule1_txtLOGINID',
                }),
                15000
            );
            let inputAcc = await iDriver.findElement({
                id: 'ctl00_loginModule1_txtLOGINID',
            });
            let inputPwd = await iDriver.findElement({
                id: 'ctl00_loginModule1_txtLOGINPWD',
            });
            ...
            try {
                let selectIden = await iDriver.findElement({
                    id: 'ddlSystype',
                });
                await iDriver.executeScript(
                    `$("#ctl00_loginModule1_hidSystype").val("student").change();`
                );
                await iDriver.executeScript(
                    `$("#ddlSystype").val("student").change();`
                );
                await submitBtn.click();
            } catch (e) {}
...
async function getCoursePlanPageUrl(driver) {
    ...
    await driver
        .navigate()
        .to(
            'https://system8.ntunhs.edu.tw/myNTUNHS_student/Modules/Teachplan/qry/Teachplan_qry_01.aspx'
        );
    ...
    let queryFrame = await driver.findElement({
        id: 'ctl00_ContentPlaceHolderQuery_QueryFrame',
    });
    ...
    let listFrame = await driver.findElement({
        id: 'ctl00_ContentPlaceHolderList_ListFrame',
    });
    await driver.switchTo().frame(listFrame);
    await driver.sleep(1000);
    let $ = cheerio.load(await driver.getPageSource());
    let url = `https://system8.ntunhs.edu.tw/myNTUNHS_student/Modules/Teachplan/qry/${$(
        '#form1'
    ).attr('action')}&index=0`;
    console.log(url);
    return url;
...
async function crawlCoursePlan(driver) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        $('a').each((index, element) => {
            ...
            if (
                (href.includes('system8') && href.includes(semVal)) ||
                ...
                if (filenameSplit.length == 4) {
                    let planObj = {
                        ...
                        secondCode: filenameSplit[3].substring(
                            0,
                            filenameSplit[3].indexOf('.')
                        ),
                        src: href,
                    };
                    ...
                    let planObj = {
                        openTeacher: filenameSplit[1],
                        code: filenameSplit[2].substring(
                            0,
                            filenameSplit[2].indexOf('.')
                        ),
                        src: href,
                    };
                    coursePlanObjList.push(planObj);
    ...
    fs.writeFileSync(
        ...
        JSON.stringify(coursePlanObjList, null, 4)
    );
}

async function getCoursePlan() {
    let item = fs.readFileSync(path.join(__dirname, 'plan.json'), {
        encoding: 'utf-8',
    });
...
async function getFaculties() {
    let item = fs.readFileSync(path.join(__dirname, 'Faculties.json'), {
        encoding: 'utf-8',
    });
    let itemJson = JSON.parse(item);
    return itemJson;
...
async function updateCourse() {
    ...
    let dataJson = xlsx.utils.sheet_to_json(courseXlsData.Sheets[sheetsName], {
        range: 4,
    }); //The couese excel header row is 5
    ...
    for (let item of dataJson) {
        let hitCourse = _.filter(coursePlans, (v) => {
            if (_.get(v, 'secondCode')) {
                return (
                    v.code == item['科目代碼(舊碼)'] &&
                    v.openTeacher == item['主開課教師代碼(舊碼)'] &&
                    v.secondCode == item['課表代碼(舊碼)']
                );
            }
            return (
                (v.code.trim() == item['科目代碼(舊碼)'] &&
                    v.openTeacher == item['主開課教師代碼(舊碼)']) ||
                (v.code.trim() == item['課表代碼(舊碼)'] &&
                    v.openTeacher == item['主開課教師代碼(舊碼)'])
            );
        });
        if (hitCourse.length) {
