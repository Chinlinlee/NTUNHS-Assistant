...
async function main() {
    ...
    await driver
        .navigate()
        .to(
            'https://system10.ntunhs.edu.tw/Workflow/Modules/Main/login.aspx?first=true&action=&info='
        );
    console.log('go to siginoff');
    await driver.wait(
        webdriver.until.elementLocated(
            webdriver.By.id('ContentPlaceHolderDetail_loginModule1_txtLOGINID')
        )
    );
    await driver.executeScript(
        `$('#ContentPlaceHolderDetail_loginModule1_txtLOGINID').val("${config.stuNum}")`
    );
    await driver.executeScript(
        `$('#ContentPlaceHolderDetail_loginModule1_txtLOGINPWD').val("${config.stuPwd}")`
    );
    await driver.executeScript(`$("#btnLogin").click()`);
    await driver.sleep(1000);
    try {
        await driver.wait(
            webdriver.until.elementLocated(webdriver.By.id('ddlSystype')),
            2500
        );
