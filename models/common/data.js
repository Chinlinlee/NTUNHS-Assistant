...
async function MongoExe() {
    ...
    let connection = await MongoClient.connect(
        `mongodb://${username}${password}${at}${config.MONGODB.host}:${config.MONGODB.port}/${config.MONGODB.db}?ssl=${config.MONGODB.ssl}&authSource=${config.MONGODB.authDB}`,
        {
...
module.exports.Getdata = async function (i_collection, i_query) {
    ...
    var mypromise = () => {
        return new Promise(async (resolve, reject) => {
            conn.db('My_ntunhs').collection(
                i_collection,
                async function (err, o_collection) {
                    var itemarray = [];
