...
const staticScoreCategory = ['90~100' , '80~90', '70~80', '60~70', '50~60', '40~50', '30~40', '20~30', '10~20', '0~10']
                ...
                if (hitCourse) {
                    try {
                        ...
                        collection.findOne({
                            $and : [
                                {
                                    courseNormalId : hitCourse.courseNormalId
                                } ,
                                {
                                    courseSem : data.Sem
                                }
                            ...
                            let courseScoreCategory =  getScoreCategory(data.score);
                            if (err) {
                                console.error(err);
                                process.exit(1);
                            } else if (courseData) {
                                if (!courseData.scoreCategory[courseScoreCategory].includes(stuNum)) {
                                    await collection.findOneAndUpdate({ 
                                        ...
                                        $push : {
                                            [`scoreCategory.${courseScoreCategory}`] : req.session.stuInfo.stuNum
                                        }
                                    });
                                }
                            } else {
                                ...
                                storeObj.scoreCategory[courseScoreCategory].push(req.session.stuInfo.stuNum);
