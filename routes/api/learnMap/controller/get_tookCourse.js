...
module.exports = async function (req, res) {
    let stuAllCourse = await getTookCourse(req)
    if (stuAllCourse) {
        return res.send(stuAllCourse)
    }
    return res.status(401).send()
}

async function getTookCourse(req) {
    return new Promise(async (resolve) => {
        let stuAllCourse = []
        let j = getILMSJar(req)
        let fetchCookie = require('fetch-cookie')(fetch, j)
        let ilmsAllCourseRes = await fetchCookie(
            'https://ilms.ntunhs.edu.tw/home.php?f=allcourse'
        )
                ...
                if (index % 5 == 4) {
                    let courseObj = {
                        ...
                        courseName: $('td')
                            .eq(index - 3)
                            .text(),
                        courseURL: `https://ilms.ntunhs.edu.tw${$('td')
                            .eq(index - 3)
                            .find('a')
                            .eq(0)
                            .attr('href')}`,
                    ...
                    _.set(
                        courseObj,
                        'courseSem',
                        courseObj.courseId.substr(0, 4)
                    )
                    _.set(
                        courseObj,
                        'courseNormalId',
                        courseObj.courseId.substring(4)
                    )
                    stuAllCourse.push(courseObj)
                }
            }
        })
        if (stuAllCourse.length > 0) {
            return resolve(stuAllCourse)
        }
        return resolve(false)
    })
}

module.exports.getTookCourse = getTookCourse
