...
module.exports = async function (req, res) {
    ...
    doc.Document.Body.sections[0].root[0].root[0].root.width =
        2127 * columns.length;
    ...
    for (var i = 1; i < realrows.length; i++) {
        for (var j = 0; j < columns.length; j++) {
            ...
            if (columns[j].includes('http://')) {
                //j == columns.length -1
                var par = new Paragraph();
                var hyperlink = new Hyperlink('點我下載', i);
                hyperlink.root[0].root.id =
                    'rId' + (i + doc.DocumentRelationships.RelationshipCount);
                doc.docRelationships.createRelationship(
                    i + doc.DocumentRelationships.RelationshipCount,
