...
function getIsOnlyQs(queryParams) {
    if (queryParams.IsOnly) {
        ...
        CourseOtherQs = _.isArray(CourseOtherQs)
            ? CourseOtherQs
            : [CourseOtherQs];
        FacultyNameQs = _.isArray(FacultyNameQs)
            ? FacultyNameQs
            : [FacultyNameQs];
    ...
    return new Promise(async (resolve, reject) => {
        ...
        Object.keys(queryParams).forEach((element) => {
            if (element != '$and' && element != '$or') {
                if (
                    !queryParams[element]['$in'] &&
                    !_.isPlainObject(queryParams[element])
                ) {
                    queryParams[element] = _.isArray(queryParams[element])
                        ? queryParams[element]
                        : [queryParams[element]];
...
function getFacultyAndEduTypeQs(queryParams) {
    ...
    if (FacultyQs && EduTypeQs) {
        ...
        queryParams['$and'] = new Array(
            { Faculty_Name: { $in: FacultyQs } },
            { Faculty_Name: { $in: EduTypeQs } }
        );
        delete queryParams['EduType'];
        delete queryParams['Faculty'];
    } else if (FacultyQs) {
...
function getPartQs(queryParams) {
    if (
        (queryParams['IsRootPart'] == 'true' &&
            queryParams['IsCityPart'] == 'true') ||
        (queryParams['IsRootPart'] == 'false' &&
            queryParams['IsCityPart'] == 'false')
    ) {
        delete queryParams['IsRootPart'];
        delete queryParams['IsCityPart'];
        if (queryParams['Course_Other'] != undefined) {
            queryParams['Course_Other'] = _.isArray(queryParams['Course_Other'])
                ? queryParams['Course_Other']
                : [queryParams['Course_Other']];
            //var regex = ToRegexText(queryParams, 'Course_Other');
