...
module.exports =
    ...
    function (app, passport) {
        ...
        app.post(
            ...
            function (res, req) {
                if (
                    req.req.headers['referer'] != undefined &&
                    req.req.headers['referer'].includes('Course_Search')
                ) {
                    req.redirect('/Course_Search');
                } else if (
                    req.req.headers['referer'] != undefined &&
                    req.req.headers['referer'].includes('historyScoreChart')
                ) {
                ...
                async function (err, user, info) {
                    ...
                    req.logIn(user, function (err) {
                        ...
                        if (
                            ...
                            req.headers['referer'].includes('Course_Search')
                        ) {
                            return res.json(req.session.stuInfo);
                        } else if (
                            req.headers['referer'] != undefined &&
                            req.headers['referer'].includes('historyScoreChart')
                        ) {
                            let refererSplit =
                                req.req.headers['referer'].split('/');
                            let redirectUrl =
                                refererSplit[refererSplit.length - 1];
                            return res.json(req.session.stuInfo);
                        } else {
                            return res.json(req.session.stuInfo);
                        }
        ...
        app.post('/api/lineid', async function (req, res) {
            ...
            var item = await data_log.Getdata('Students', {
                lineid: instance.id,
            });
            if (item.length < 1) {
                res.send('no lineid');
        ...
        app.get('/History_Scores', My_Func.IsLoggedIn, function (req, res) {
            ...
            res.render('./atm/History_Scores.html', {
                user: req.user.toString(),
            });
        });
        ...
        app.route('/api/log', My_Func.IsLoggedIn).all(async function (
            req,
            res,
            next
        ) {
            ...
            switch (req.method) {
                case 'GET':
                    ...
                    var items = await data_log.Getdata('Schedule', {
                        user: instance.User,
                    });
                    var Result = [];
                    if (items.length >= 1) {
                ...
                case 'POST':
                    if (instance.method === 'getnewdata') {
                        ...
                        //將讀取的資料轉成Json
                        ////將Json前面加入擁有者的學號
                        var Schedule_jsondata = fs.readFileSync(
                            '../../Schedule/' + instance.User + '.json',
                            'utf8'
                        );
                        var Schedule = JSON.parse(Schedule_jsondata);
                        var Schedule_json = { user: instance.User, Schedule };
                        var Courses_jsondata = fs.readFileSync(
                            '../../Courses/' + instance.User + '.json',
                            'utf8'
                        );
                        delete Courses_jsondata[0];
                        var Courses = JSON.parse(Courses_jsondata);
                        var Courses_json = { user: instance.User, Courses };
                        var Score_jsondata = fs.readFileSync(
                            '../../Scores/' + instance.User + '.json',
                            'utf8'
                        );
                        var Scores = JSON.parse(Score_jsondata);
                        var Score_json = { user: instance.User, Scores };
                        var HS_jsondata = fs.readFileSync(
                            '../../AllScores/' + instance.User + '.json',
                            'utf8'
                        );
                        var History_Con_Scores = JSON.parse(HS_jsondata);
                        var HS_All_jsondata = fs.readFileSync(
                            '../../AllScores/' + instance.User + '_All.json',
                            'utf8'
                        );
                        var History_Scores = JSON.parse(HS_All_jsondata);
                        var History_Scores_json = {
                            user: instance.User,
                            History_Scores,
                            History_Con_Scores,
                        };
                        var Stu_Inf_jsondata = fs.readFileSync(
                            '../../Stu_Num/' + instance.User + '_inf.json',
                            'utf8'
                        );
                        ...
                        //有:更新
                        //沒有:插入
                        var IsExist = await data_log.Getdata('Schedule', {
                            user: instance.User,
                        });
                        if (IsExist.length >= 1) {
                            result = await data_log.Updatedata(
                                'Schedule',
                                { user: instance.User },
                                { $set: { Schedule: Schedule_json.Schedule } }
                            );
                            result = await data_log.Updatedata(
                                'Courses',
                                { user: instance.User },
                                { $set: { Courses: Courses_json.Courses } }
                            );
                            result = await data_log.Updatedata(
                                'Scores',
                                { user: instance.User },
                                { $set: { Scores: Score_json.Scores } }
                            );
                            result = await data_log.Updatedata(
                                'History_Scores',
                                { user: instance.User },
                                {
                                    $set: {
                                        History_Scores:
                                            History_Scores_json.History_Scores,
                                        History_Con_Scores:
                                            History_Scores_json.History_Con_Scores,
                                    },
                                }
                            );
                        } else {
                            result = await data_log.Insertdata(
                                'Schedule',
                                Schedule_json
                            );
                            result = await data_log.Insertdata(
                        ...
                        result = await data_log.Updatedata(
                            ...
                            { $set: { Inf: Stu_Inf_json } }
                        );
                        var UpDay = await Get_Date_YYYYMM();
                        result = await data_log.Updatedata(
                            'Students',
                            { username: instance.User },
                            { $set: { Last_Up_Time: UpDay } }
                        );
                        result === 'fail'
                            ? res.send('fail')
                            : res.send('success');
                        ...
                        var userpwd = '';
                        if (
                            req.session.userpwd != null ||
                            req.session.userpwd != undefined
                        ) {
                        ...
                        var cryptpwd = Cryptjs.AES.encrypt(
                            userpwd,
                            'jhytjtrfewfwfrehtergwe'
                        );
                        var cryptpwd_2 = Cryptjs.AES.encrypt(
                            cryptpwd.toString(),
                            'jkyro0gkeprof,ewopf,owep'
                        );
                        var cryptpwd_3 = Cryptjs.AES.encrypt(
                            cryptpwd_2.toString(),
                            'yureoqrueiwqrieowqprwep'
                        );
                        var cryptpwd_4 = Cryptjs.AES.encrypt(
                            cryptpwd_3.toString(),
                            'hotropwgjreiognehoie'
                        );
                        let encData = Cryptjs.enc.Base64.stringify(
                            Cryptjs.enc.Utf8.parse(cryptpwd_4)
                        );
